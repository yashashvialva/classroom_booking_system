...
if(isset($_POST['book'])){
    ...
    $stmt = $conn->prepare("INSERT INTO bookings (room_id, faculty_id, reason, date, start_time, end_time, status) VALUES (?,?,?,?,?,?, 'pending')");
    $stmt->bind_param("isssss", $room_id, $faculty_id, $reason, $date, $start_time, $end_time);
    $stmt->execute();
    $stmt->close();
